...
- **DEFERRED — UI list reads still cap at 1000.** `entities/index` (entities + `is_a` + `icp_fit`), `feed/list` (`icp_fit`), and the intake-tool rankings read workspace-wide with no limit. Cosmetic until a workspace exceeds ~1000 entities (then the list/ranking silently truncates). Fix before onboarding any workspace that large. `research.ts:99` left alone (per-entity bounded, can't exceed 1000).
- **Systemic root worth a guard:** any `.select()` scoped only by `workspace_id` with no `.limit()` is suspect — the result is only safe if you genuinely want ≤1000 arbitrary rows, which a membership/count/dedup check never does. A lint rule banning the shape would stop this class recurring (offered, not built).
