...
## 2026-07-28 (later still) — demo workspace: un-restored and re-archived the 2,543 wrongly-hidden entities

Earlier session context (compressed before this one): on 2026-07-24 a bug in `entityArchiveSweep`'s activity check (1000-row `.in()` cap, fixed `00ae273`) had wrongly auto-archived 2,543 real, fact-bearing companies in the demo workspace (af602fa1). A restore script (`_restore_wrongarchive.ts`) existed to fix it but had not been run as of the 07-24 wrap.

Earlier today that restore was run (un-archived all 2,543). Then found the explicit 07-24 note: Jake had already applied the same fix on Sudden and deliberately left demo's hidden companies untouched, since demo is parked and not being worked. Conflict flagged; Jake said put demo back exactly as it was.

Built `_rearchive_wrongarchive.ts` to reverse the restore: no event-log row exists for the original raw `.update()` (bypassed `record_event`), so the 2,543 were isolated by `archived_at IS NULL AND updated_at` clustered in the restore's own write window (all 2,543 landed in a 3-second span, 2026-07-28T12:57:38-41, nothing else in the workspace touched that day). Ran with `--cutoff 2026-07-28T12:56:30 --go`: **2,543 re-archived**, active count back to 199 (matches pre-restore baseline).

**Worth keeping straight:** this is not the same thing as the demo pipeline pause (`pipeline_paused`, scope=all, manual, reserving shared quota for the Sudden CSV burst — see `feedback_dogfood_pause_intentional`). That's about whether the agent runs. This was about `archived_at` — whether the 2,543 companies render in the UI at all. The underlying bug is fixed and, left to logic alone, these 2,543 *should* be visible (they're real accounts, not junk). They're hidden only because Jake wants demo untouched while parked, not because hidden is the correct end state. **If this surfaces again — e.g. someone notices 2,543 real-looking companies archived in demo and assumes it's still the sweep bug — it is not. It's this deliberate re-archive. Do not restore them again without asking.**

Both scripts (`_chk_restore_timestamps.ts`, `_rearchive_wrongarchive.ts`) left untracked in `scripts/`, same as their `_restore_wrongarchive.ts`/`_chk_wrongarchive.ts` predecessors — fine to delete, not committed.


## 2026-07-28/29 — recap sweep + a real audit of research/enrichment, not draft writing

Ran `pnpm recap` (sweep + daily report) on Sudden and read it end to end instead of trusting the sweep's RED/YELLOW alone. Both current RED items turned out to be already-diagnosed, non-actionable: `score_distribution` (65% in decile 8/10) is the known outcome of `[[project_score_collapse_sudden]]` — the two fake dimensions are fixed, the rest is the book, Jake's weights call. `cron_stale:ats_hiring_main` on the demo workspace is a symptom of the intentional pause ([[feedback_dogfood_pause_intentional]]). Neither got touched.

The report's "What's next" section had three real leads. Chased all three before writing anything.

**1. Domain backfill was the actual bottleneck for the whole enrichment chain, not just contacts.** 1,396 of Sudden's 1,995 accounts (70%) have no `attributes.domain`, draining at a fixed 75/day (~19 days). Traced why that matters beyond contact pulls: `resolveDomainViaSearch`'s own comment says it plainly — "research skips any account without one, and research is what lifts signal_strength past the draft gate." Today's advance-pass breakdown backs this up: 365 of 400 scanned accounts were `below_draft_gates`, and the one contact-pull attempt failed for lack of a domain. Domainless is upstream of own-site research, which is upstream of evidence_depth/signal_strength, which is upstream of draft gates, which is upstream of contact pulls. One knob gates the whole book.

Checked whether raising it is safe before touching it: `runDomainBackfillBatch` already self-pauses the workspace's research scope on a real Exa credit/402 error (same pattern as the DeepSeek 402 wall), so a higher daily count can't silently overspend past what's actually available — worst case it hits the wall and pauses with a plain reason, same as today. At Exa's per-search pricing (~$0.01/domain search incl. content pages), raising `policy.research.domain_backfill_per_day` 75 → 150 costs an extra ~$0.75/day and roughly halves the clear-out time to ~9-10 days. Applied directly to Sudden's policy (`e7052848`) via read-modify-write on `workspaces.policy`, same pattern as `setPipelineStatus` — doesn't touch `scoring_config_state.changed_at` so it can't trigger a rescore.

**2. There was a free resolver that only ever ran once.** `backfillAccountDomainsFromContactEmails` (`packages/tools/src/domains.ts`) derives an account's domain from a linked contact's corporate email — zero LLM calls, zero Exa credit, precision-gated (name-match required, conflicting hosts skipped). It's wired into the CSV-import route and existed as a one-off script (`_backfill_sudden_domains.ts`), but nothing in the daily pipeline ever called it again after initial import. Every domain resolved since then paid for an Exa search even when a free email-based answer might have existed. Dry-ran it against Sudden first to check for current yield: **0** — the 71 accounts with contact emails were already fully resolved or correctly rejected (8 name-mismatches like a "SOOP" contact at `amazon.com`, correctly skipped). So it wasn't costing anything *today*, but it also wasn't going to catch anything *tomorrow* when Hunter finds a new contact for a still-domainless account. Wired it into `domainBackfillDaily` (`inngest/functions/system_tasks.ts`) as a free first step, every workspace, before the paid scan — shrinks what the paid path has to spend on, automatically, from here on. This is the "learns over time, costs less" behavior the system should have by default, not something that needs re-running by hand.

**3. Contact-pull volume (1 attempt in 24h) is a real vendor cap, not a bug.** `policy.enrichment.hunter_monthly_cap: 50` — that's ~1.6 lookups/day averaged over a month, almost certainly mirroring Hunter's actual plan tier rather than an arbitrary internal choice (same shape as the Explorium/DeepSeek credit walls already in memory). Did not touch this: raising a monthly cap that reflects a real paid-plan ceiling would just move the failure from "capped" to "Hunter API rejects it," and switching providers is the already-tracked, bigger decision in [[project_contact_source_vibe_over_hunter]] (needs a new API key + connector, Jake's call, not a code fix). Left as a documented constraint, not re-flagged as new.

Considered and explicitly rejected fixing the fourth number in the report — "100 of 2,287 score writes (4%) re-judged unchanged evidence." Read `scoreEntity`/`scoreAndAssert` in `packages/tools/src/scoring.ts` closely: the code already has two guards against this (a read-time reuse check at the top of `scoreEntity`, a write-time race re-check right before the actual write), and the comments on both explicitly document that they *shrink* a real TOCTOU race between concurrent scorers (agent_run's per-entity Inngest concurrency lock only covers agent_run itself — the 30-min rescore cron and `contacts.ts` call `scoreAndAssert` directly, outside any lock) but don't fully close it — "5 simultaneous runs produced 2 score rows" is called out by name in the existing comment. Closing it for real needs either a DB-level atomic conditional write (a new Postgres RPC replacing the check-then-`act()` pattern used everywhere facts get superseded) or restructuring the cron/contacts paths to fan out through the same per-entity Inngest concurrency key as agent_run — both are structural changes to a primitive used all over the codebase, for a residual, already-mitigated, non-alerting 4%. Left it; noted here so it doesn't get re-diagnosed as new.

`pnpm verify` clean after the `system_tasks.ts` change. `_set_domain_backfill_rate.ts` and the (now-deleted) `_chk_domain_backfill_policy.ts` were throwaway, same convention as every other `_chk_*`/`_set_*` one-off in `scripts/` — not committed.


## 2026-07-29 (later) — own_site research signals were exempt from the freshness gate; fixed + purged

Jake pasted a batch of "PAIN" facts from Sudden's chat and flagged one as sourced from a 2021 launch announcement, then set an explicit goal: research signals should be no more than a month old.

**Traced the mechanism instead of guessing.** `inngest/functions/research.ts`'s post-fetch age gate had a hard exemption: `if (angle.domain_scope !== 'own_site' && er.publishedDate)` — own_site results skipped the freshness check entirely, on the documented (but never verified) assumption that a company's own pages are "timeless" and that date-filtering an own-domain search "tends to return nothing." Separately, `buildAngleRequest`'s own_site branch never sent `start_published_date` to Exa at all, even when the angle declared a `recency_days` window — so nothing bounded own_site queries at either the request or the post-fetch stage.

Confirmed live before touching code, not just from the code reading. Queried Sudden's `signals` table directly (paginated past the 1000-row PostgREST cap): of 609 dated `research_result` signals sampled, 276 were over 180 days old, almost all under `own_site_scaling`/`customers` angles, and new ones were still landing same-day — a `titanchannel.com` "estreno" (launch) post dated 2021-03-03, ingested 2026-07-29. Exact match for Jake's complaint.

**Fixed in `inngest/functions/research.ts`, `research_strategy.ts`, `policy.ts` (`209666c`, pushed):**
- Age gate now applies to every scope; only a genuinely undated result stays exempt (unchanged for real evergreen pages — customer lists, general product pages).
- own_site queries now send `start_published_date` to Exa when the angle sets `recency_days`.
- `DEFAULT_MAX_AGE_DAYS` 365 → 30 days. `BASELINE_ANGLES` and the strategy-planner `SYS_PROMPT` tightened to 30-day windows everywhere instead of 90-365.
- Patched Sudden's live `policy.research.strategy` directly (own_site_scaling 365→30, news_coverage 180→30, social_posts 60→30, `max_age_days: 30` explicit) so the fix took effect immediately instead of waiting up to 14 days for the strategy cache to regenerate on its own.

`pnpm verify` clean (full monorepo typecheck + assertion suites) before commit.

**Found a second, different bug in the same audit and fixed it too.** One of the pasted PAIN facts — Love Nature's "BXF format" ingestion pain — traced to an *undated* case study on `amagi.com` (a competing CDN/playout vendor) describing how Amagi solved that exact problem for a *different* company (Blue Ant Media, which owns the Love Nature channel). Not a dating problem — the freshness gate correctly can't touch an undated page. It's an identity/relevance gap: the relevance check confirms "does this page mention the target," not "is the target the actual subject of the page." Tightened the `filterResultsByEntity` system prompt in `research_strategy.ts` to explicitly reject a page whose real subject is a different company, where the target is only named as that company's customer/channel/brand. This is a prompt change, not a deterministic filter — unverified in production, flagged in `project_state.md` as a watch item.

**Purge, scoped carefully after a clarifying question.** A blanket "delete every fact touching a stale signal" would have hit 1,275 facts, including 64 pipeline-bookkeeping facts (`outreach_stage`, `icp_fit_breakdown` — confirmed via the existing `ADMIN_PREDICATES` set in `scoring.ts`) whose deletion would have silently reset account pipeline stage. Split the remainder into 243 time-sensitive trigger/pain facts vs. 968 static profile facts (founder, HQ, employee count, description — still true regardless of source-article age) and asked Jake which to purge, given this exact codebase has a recorded prior incident of over-aggressive fact deletion needing a revert (`[[project_supersede_fork_bug]]`). He chose the conservative option. Deleted exactly 243 facts; left the 968 static facts and all 64 bookkeeping facts untouched; did not touch the `signals` table at all (many stale signals still back the retained static facts).

Noticed a second `claude` process (PID from an earlier terminal, unrelated session) actively editing files in the same working tree mid-session — a score-evidence-citation feature (`evidence_fact_ids` in `scoring.ts`/`graph.ts`, several `apps/web` files). No file overlap with this session's changes; staged and committed only the 3 files this session actually touched rather than `git add -A`, so nothing from the other session's in-progress work got swept in.

New memory: `project_research_own_site_age_exemption_bug.md`.


## 2026-07-29 (later still) — fact citations wired platform-wide: nothing shown was traceable back to its source

Jake pasted a Today-page mover card (ViX, network proximity 0.00→0.70, a "new info" claim, several fact chips) and said the citations need to link to the specific fact and its actual source. Then broadened it: "every reference in the platform needs to be a link. No isolation anywhere" — followed by a session-scoped Stop hook with the same condition, so this became a full audit, not a one-page fix.

**Two research passes (Explore agents) mapped the whole surface before any edit.** First pass: the Today page's `MoverCard`/`LearnedCard` (`apps/web/app/_lib/today.ts` + `TodayClient.tsx`) already had fact `id`s sitting in the query result (`FactRow.id`) and just dropped them before the UI — `TodayMove.facts` was typed `{predicate, object}` with no id, and `TodayMove.claim.cites` (already a real `uuid[]` of fact ids per the `channel_posts.cites` schema) was fetched but never rendered. Also found the provenance stack already exists and is used elsewhere (`CiteChain.tsx`, `WhyThis.tsx`, `/api/facts/batch`, `/api/facts/[id]/chain`, `resolveSourceForFacts`/`resolveSourceSignalsBatch`) — the pattern to replicate, not build from scratch.

Second pass (platform-wide) found the same "id present, dropped before the UI" shape repeating independently in six more places, because there's no shared fact-row component — each page hand-rolls its own: EntityDetail's score-breakdown card, its audit/replay time-slider and raw event stream, its relationship graph (dropped `via_fact_id` on every edge), Chat's `QueryFacts`/`QueryEntities` tool-result renderers, the top-level Replay page's posts/signals/score-bars, and the sources page's signal drawer (entity name wasn't even a `<Link>`). One genuinely different case: the "moved on network proximity" driver line had no fact id anywhere, not even in the backend — `graphProximity()` (`packages/tools/src/graph.ts`) computes the mean from specific neighbor `icp_fit` facts but discarded their ids before returning.

**Fixed all of it in one pass, 11 files:**
- `today.ts`/`TodayClient.tsx`: `TodayMove.facts`/`TodayLearned.facts` now carry `id`; `claimsOf` now carries `post_id`/`created_at` (available for free from the existing posts loop) so the claim can drive a real `WhyThis` panel; mover cards' fact chips get a `CiteChain` trace pill each.
- `graph.ts`: `GraphProximityResult` now returns `evidence_fact_ids` (the edge fact + neighbor `icp_fit` fact that fed each contribution to the mean).
- `scoring.ts`: threaded `evidence_fact_ids.graph_proximity` through all four `ScoreBreakdown` construction sites (reused-judgment path, RRF-shortcut path, full LLM path, contact scorer) and it rides along in the existing `...score.breakdown` spread into the persisted `icp_fit_breakdown` JSON — no new write needed. `today.ts`'s driver computation now reads it back off the breakdown and attaches `fact_ids` to `TodayMove.driver`, so the network-proximity delta finally has real facts under it.
- `EntityDetail.tsx`/`RelationshipGraph.tsx`: score-card breakdown, audit-slider active facts, raw timeline's fact-kind rows, and every relationship-graph edge now carry a `CiteChain` trace (the graph's edge labels get a small non-clipped "connections" list below the SVG diagram, since the diagram itself is `overflow:hidden` and an expanding trace panel would get cut off there).
- `Chat.tsx`: `QueryFacts` and `QueryEntities`'s single-entity view now render `CiteChain` per fact (`QueryFacts` already had the id in hand, just unused as anything but a React key).
- `api/agent/intake/tools.ts`: `queryEntities`'s single-id branch already selected `id` in the query — the frontend type just didn't declare it. While in this file, also fixed a stale-fact bug: `.is('supersedes', null)` returns the pre-supersede original, not the current fact (same class already fixed at `loadBestContactScore` and the feed `icp_fit` read per the running tracker in `project_state.md`) — rewrote to the `excludeSuperseded`-style exclude-pointed-to-ids pattern.
- `api/replay/summary/route.ts`/`replay/page.tsx`: `top_posts` never selected `cites` at all (no `WhyThis` was possible); added `cites` + resolved the `decision`-child-post-as-reasoning convention from `feed_items.ts` so it renders properly. `top_signals` gained a real source URL via the existing `pickSourceUrl` helper. `newest_entities`' per-dimension score bars now carry the backing fact id and a `CiteChain`.
- `sources/page.tsx`: signal drawer's entity name is now a `Link` to the entity page.

**Verification:** `pnpm verify` (full 8-package typecheck + `check_score_formula`/`check_domain_guard` assertion suites) clean after one fix (a `driver?.key === 'graph_proximity'` optional-chaining narrowing that TS wouldn't carry through a spread — rewrote as an explicit `driver &&` guard). Smoke-tested the touched routes with curl (307 to `/login`, confirming middleware-level wiring with no server error, not a real UI check). Could not do a live browser walkthrough — the Chrome extension wasn't connected in this environment. Flagged as a next-session watch item in `project_state.md`, along with the fact that `evidence_fact_ids` only populates on freshly-computed breakdowns, so existing accounts won't show the graph_proximity trace link until their next rescore.

Committed `d8db933`, pushed to `origin/main` (Render auto-deploys). Noticed mid-session that a second `claude` process was concurrently editing `research.ts`/`research_strategy.ts`/`policy.ts` in the same working tree (a different session, the own_site freshness-gate fix above) — no file overlap, staged only the 11 files this session actually touched rather than `git add -A`.


## 2026-07-31 — a date the model could read was thrown away because nobody told it the format

Jake arrived with a diagnosis already written (a prior session's analysis of a French press release whose signal kept a crawl date) and asked me to evaluate it, implement it, or fix its gaps. It named two things: the enricher prompt never tells the model to convert a non-ISO date it finds, and `parseContentDate` rejects non-ISO silently. Both held up on inspection.

**Verified before touching anything.** `packages/tools/src/published_date.ts:146` matches `^((?:19|20)\d{2})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$` and returns `null` on anything else, indistinguishable from a page that carried no date. The prompt (`agent_logic.ts:1473`) said "in YYYY-MM-DD form" and listed "a press-release header" as a place to look, so it pointed the model straight at the line it then couldn't format. Also checked the truncation theory the analysis had ruled out and confirmed it independently: `buildUserPrompt` does `JSON.stringify(signalForPrompt)` over the whole signal, no slice, so the header was in front of the model.

**The gap in the proposal: "tell it to convert" moves the guess into the model rather than removing it.** `23/04/2026` is unambiguous because 23 can't be a month, but a page printing `04/07/2026` is April 7 or July 4 depending on where it was written, and a prompt that says only "convert to YYYY-MM-DD" gets a coin flip on those. Added the ordering rule alongside the conversion rule: read day/month order from the page's own language and country (French/Spanish/German/British English put the day first, US English the month), any number above 12 can only be the day, and return "" when both are 12 or under and nothing settles it. That last clause matches the rule already in the block ("do not estimate, infer from context, or guess a year") and costs one undated source instead of recording a wrong date — the same asymmetry `applyContentDate` is already built around.

**Second refinement on the logging half.** Logging every `parseContentDate` rejection would fire on `"unknown"` and `"n/a"`, which is the model correctly reporting the page said nothing, and the noise would bury the real cases. New `unreadableContentDate()` splits them on whether the value carries a plausible four-digit year: with one, it's a date we failed to read and gets written to `signals.structured_tags.published_at_unreadable` (capped at 64 chars) plus a `console.warn`; without one, it's the model answering in words and stays unrecorded. Restructured the enricher write-back into `if (unreadable) … else if (corrected)` rather than merging both into the existing update, since the two are mutually exclusive but a reader shouldn't have to derive that.

Files: `published_date.ts` (new function + export), `packages/tools/src/index.ts`, `agent_logic.ts` (prompt block + write-back branch + import), `scripts/check_published_date.ts` (8 assertions including the live `23/04/2026` case, and the negative cases that keep the year-heuristic honest). `pnpm verify` clean — 8-package typecheck plus all three assertion suites.

Committed `79f27ab`, pushed to `origin/main`. Left the two untracked `_`-prefixed scratch scripts in the tree alone rather than staging them.

**Known limit, recorded rather than papered over:** this is forward-only. The raw LLM output was never persisted anywhere, so signals already carrying a wrong provider date have nothing to recover the real date from. Offered a one-off diagnostic pass over existing signals (flag where `published_at_source` is the provider and the body holds a disagreeing date-shaped string); Jake didn't take it, so it's logged in `project_state.md` as not built. The prompt edit also changes `prompt_hash`, so enricher calls hash differently from the next run forward.


## 2026-08-04 — a vendor we cannot serve scored 0.87 and got drafted; five fixes upstream of the writing

Jake opened with a bad LinkedIn draft and a diagnosis question: is the robotic tone a model problem or an instructions problem, and why does every draft lean on the same 1:1 cost pitch. The writing turned out to be the last problem in the chain rather than the first, so most of this session is upstream of it.

**The draft was to TVU Networks, and the account should never have been drafted.** TVU sells live video production and cloud infrastructure to broadcasters; they do not pay a viewer delivery bill, their customers do. The message opened "CHZZK on TVU One at 100K concurrent viewers" — CHZZK is TVU's *customer*, TVU One is TVU's *product*, and the traffic is live, which is out of scope. It also landed on top of an open advisor conversation (`prospect_notes`: "he's interested in advisor role"). It scored `industry_match 1.00` / `icp_fit 0.87`; the scorer read "media technology company" and matched on industry, with no way to express "they sell what we sell."

**Scope, as a veto.** New `policy.drafter.out_of_scope`, empty by default, read by the scorer (forces `icp_total` to 0) and the drafter (STEP 0 → `request_gate`, which covers accounts scored before a condition existed). It has to veto rather than be averaged, and `scripts/check_out_of_scope.ts` pins why: TVU's real sub-scores combine to 0.78, or 0.71 with an entire dimension zeroed, against a 0.40 draft bar. No weighting scheme could have stopped it. Guards: the rubric answers with a 1-based condition NUMBER and an out-of-range index is dropped, so an invented condition cannot delete a real prospect; a stored veto survives the score-reuse path. Rescored 1961 accounts: **18 vetoed (0.8%), no false positives** — AWS, Akamai, Level3, Edgio, CDN77, JW Player, Kaltura, Wowza, muvi, VPlayed, Caylent, Intel, TVU, Netskrt Systems (peer-assisted delivery, a direct competitor), plus Willow/More Than Sports TV/Walta Info on live-only. A third condition ("reaches viewers only through native/TV apps") was written and deleted before the rescore: the binary test can never fire, since essentially every streamer has a web player, and the accounts it targeted have one too.

**Why every draft was the same pitch — three causes, not one.** (1) `pain_points`/`value_props` are derived from ABOUT at setup, stored, passed into `buildDrafterDecision`, and dropped by the templates branch before rendering; the exemplars were the only content the model had. Now rendered as a menu after the exemplars. (2) The exemplar bodies were finished video messages carrying a finished QUESTION, so picking a template picked its question — MBC produced a synonym swap of template 2's fork with the content menu already in place. Rewriting all four bodies into an unrelated vertical (freight) stopped it: the shape still transfers, the content cannot be copied. (3) `prompt_builders.ts` STEP 3 taught the fork using the exact 1:1 question every draft was reusing. Worth keeping straight: the CRED repeating is correct and intended (the constitution allows exactly two claims, deliberately), the QUESTION repeating is the defect.

**A rule that produced the thing it banned.** STEP 6 read `no trailing fragments like "Shows in your dashboard."` and a draft ended with exactly that sentence. Quoting a banned string that would be usable output in the slot the rule governs hands the model the string. Restated positively; the file header now records when quoting is still right (input formats, prescribed CTAs, industry clichés the model would reach for anyway). Also removed the video-vertical examples from shared craft code, which renders for every workspace, and added a rule binding the FIRST sentence specifically — both bad drafts opened with a verbless noun pile and "open on them" never bound the opener.

**Dating a fact must not delete it.** Asked to fix dates upstream so Intigral would draft again, the literal fix made it worse: `publishedDateFromUrl` already existed and worked, the signals just predated it by one day, so a backfill dated Intigral's VisualOn case study to 2025-03 — past `trigger_max_age_days` — and the old "facts older than 90 days are dead weight in both modes" rule deleted its second source. Age now retires EVENTS; a fact describing how a company stands (what it runs on, who it buys from) survives as theme evidence. Also stated outright that a theme may rest entirely on undated facts, which matters because **75% of Sudden's signals carry no date and 61% have no URL** — theme-led mode is the only route most of the book has. The URL backfill itself recovered 7 of 4208.

**The config trap.** `scoring_config_state.changed_at` was the only thing that could get an unchanged account past the skip-when-stale guard, so the first rescore attempt no-opped on ~3/4 of the book because the config script didn't bump it. Fixed in that script, then designed out properly: the guard now also compares the live `inputs_hash` against the stored one, which already fingerprints icp/about/persona/out_of_scope, so no writer has to remember anything. Costs nothing — a matching hash falls through to the reuse path with `llm_called=false` (verified on MBC and Weyyak).

**Contacts, and a harness that was lying.** Every dry run reported "0 contacts" for every account, which read as a coverage problem. Partly it was — but `create_contact` links via a `works_at` FACT while both dry-run scripts queried `attributes.works_at`, which nothing sets. Fixed, and neither requires an email now, since the linkedin channel never uses one. Separately, `scripts/extract_contacts_from_notes.ts` promoted **50 contacts across 48 accounts** out of imported note text (accounts with a contact: ~71 → 119), no provider and no cap. The prompt is deliberately strict because the notes are messy: it refuses intermediaries ("Through Mohsen Lhaf"), people the note says have left, bare pronouns, and a name pasted beside someone else's profile URL. Cineverse went from undraftable to a draft addressed to Tony Huidor by name.

**Facts about a person filed against the company.** Minutus Computing scored `industry_match 0` — "a consulting firm focused on packaging and sustainability" — off a contact's LinkedIn bio sitting in `company_description`, which the ICP rubric reads. Swept: 13 of 1783 (0.7%). 12 bios moved to a new `misfiled_person_bio` (added to `ADMIN_PREDICATES`, so parking one cannot inflate the `evidence_depth` of the account it was polluting) and one hand-written sales note moved to `prospect_notes`. Superseded, not deleted. `pain_observed` and `prospect_notes` swept clean, so the misfiling was confined to the one prose field the rubric reads directly.

Two corrections I made mid-session and should not be re-litigated: the first-touch ban on the 60-80% savings figure and pay-from-savings pricing is deliberate (constitution + `_dryrun_drafts.ts:30`), not an oversight; and stating a category-level pattern before asking the question is what an expert does, not a dead rhetorical question — the rule now only forbids asserting the answer about THIS company.

Committed `709f8f6`, merged fast-forward to main, pushed to `origin/main` (Render auto-deploys). 20 files, +1960/−26. Left alone: the two `.claude/*.md` files dirty since 07-31 and three untracked `_`-prefixed scratch scripts, none of them this session's. Full findings and the ranked remaining work are in `.claude/drafter_quality_plan.md`; the signal-extraction section there is the intended next session, since research now finds company descriptions rather than dated events.


## 2026-08-04 (later) — the drafter picks its argument before it sees an example, and the sameness moves one layer up

Continuation of the session above. `.claude/drafter_quality_plan.md` ranked one item at the top: never render an exemplar whose angle matches the one being written. Three prose rules telling the model not to copy had already failed, so this had to be structural.

**The ordering problem, and why the answer is a second call.** "Withhold the exemplar arguing what you are about to argue" is unbuildable as stated, because the model picks its argument *after* reading every exemplar. Something has to decide the argument first. Options weighed: strip every exemplar body and render anatomy only (free, but no example of a good question survives); author several exemplars per template and rotate (free, but each draft still has one angle sitting in context); or a cheap pre-call. Jake took the pre-call.

`packages/tools/src/pick_angle.ts` — one `deepseek-v4-flash` call per drafted account (the model `classifyRole` already uses; the drafter stays on `deepseek-v4-pro`, so nothing configured changed). It reads the account's facts, picks one problem from `pain_points`, and names which template angles argue the same thing. New config field `policy.drafter.templates[].angle`, one plain sentence per template. Unset = the exemplar renders in full, so a workspace that never sets it is untouched.

**Two things the first version got wrong, both found by running it rather than reading it.**

The picker was stricter than the drafter it feeds. It demanded a fact naming the problem outright; the drafter's own STEP 2 allows one inference hop (a growth number reaches a cost problem in one step). On MBC Group it returned "no problem fits" — 48 facts including audience growth and live-channel launches — and the whole feature silently no-opped back to the old prompt. Rewrote the rule to mirror the one-hop test, and to reject three-hop chains explicitly.

Then it kept choosing `pain_points[0]` for everything. Added a distinctness test ("if your reason would be equally true of every other company in this industry, you have not chosen anything; the order of the menu means nothing"). Distinct problems chosen went 2 → 3 and draft pairs over 45% word overlap went 3 → 1 across two runs of the top 8.

**A bug the change surfaced, now structurally dead.** `angle` was threaded into `buildSystemPrompt` correctly and never reached the prompt: the hand-off to `buildDrafterDecision` re-lists ~17 optional keys by hand and the new one was not on the list. Third field this call site has eaten (`templates` in `f101935`, `char_budget` caught pre-commit on 07-21) — every key is optional on the receiving type, so a dropped one type-checks clean. It spreads the object now. `_chk_drafter_prompt.ts` caught it within a minute of the change and now asserts the withheld-exemplar path with five checks.

**Silent nulls got a reason.** `pickDraftAngle` returned bare null for five different failures: menu too small, no facts, API error, unparseable JSON, and "ran fine, nothing fits". Those need opposite fixes and looked identical from outside. It returns `{ choice, reason }` now, and the reason lands on the `drafter_shortlist_pick` event beside the draft it shaped. Same trap as the harness that reported "0 contacts" for every account earlier today.

**What the measurement showed, which is not what was expected.** Withholding works — MBC Group picks the cost-per-viewer problem and `t2_founder`, the exact template it cloned this morning, renders with no body. Ab Films TV picked the redundant-egress problem and withheld `t3_technical` instead. Cineverse got the release-spike problem and produced the one genuinely different question in the batch. But five of seven accounts still land on `pain_points[0]`, which reads "Delivery cost per viewer never falls; the cost line grows one to one with the audience" — that is not a problem, it is the answer to the question every draft asks. Problems 2 and 4 are special cases of it. The picker cannot separate what the config does not separate, and once assigned that problem the drafter writes the 1:1 question with nothing left to copy from. Cutting off the copy source did not end the sameness; it revealed that the menu was choosing the content all along.

That last part is deliberately not fixed here: rewriting Sudden's four pain points into genuinely different problems is a product-claims decision, and the constitution mandates some convergence anyway (two allowed credibility claims, one of which IS the 1:1 claim).

Files: `pick_angle.ts` (new), `policy.ts`, `prompt_builders.ts`, `index.ts`, `agent_logic.ts`, `_set_sudden_template_angles.ts` (new, applied to Sudden), and the three grading scripts, which now run the picker per account instead of building one prompt for the batch — a harness that skipped it would grade a prompt that never ships. `pnpm verify` clean. Left the three untracked `_`-prefixed scratch scripts from the earlier session alone.

**Watch on the first automated cycle after deploy:** `angle_outcome` on `drafter_shortlist_pick`. Mostly `no_problem_fits` means the picker is refusing rather than choosing and every draft has quietly reverted to the old full-menu prompt.

**Follow-on the same day: the pitch menu, and the product fact that decided it.** With the picker built, the remaining convergence was in `pain_points`, so I proposed a rewrite. Jake killed half of it with one fact: offload needs viewers watching the same title at the same time, and the target is back catalogue, not live. That took out every entry built on a premiere or a live match — which I should have caught myself, since `out_of_scope` condition 1 already vetoes live-only accounts and a menu entry about live events would have aimed the drafter at exactly the accounts the veto exists to exclude. It also took out binge viewing and long-tail rewatch, both of which are viewing spread over time, and spread-out viewing forms no swarm. A region-based prop went too, because neither of us could say it was true.

`pain_points` 4 → 3, `value_props` 4 → 5 (`scripts/_set_sudden_pitch_menu.ts`, applied). The 1:1 entry is gone from the menu: it is the constitution's credibility claim, not a problem the buyer has, and leaving it on a pick-one list handed the model the answer to ask back as a question. The new value prop, "runs without anyone on the video team owning it day to day", closes a hole nobody had noticed — the old menu carried a pain about an understaffed delivery team and nothing on the other list answered it, so a draft that picked it had nothing to offer.

Re-graded the same 8: three distinct problems across six picks, two `no_problem_fits`, one draft pair over 45% overlap. Intigral now asks whether the delivery cost line has a single owner or is split across groups, Cineverse whether delivery eats the ad yield per view. Neither question was reachable from the old menu. The one draft still leading with the plain 1:1 question is idilio TV, which is precisely the account where the picker declined and the prompt fell back to the full menu — the mechanism visible in a negative case.

**A measurement I had wrong, corrected in the harness.** I graded the morning's change on "distinct problems chosen" rising 2 → 3. That is not a score. A product that does one thing for one situation should converge on one argument, and most accounts landing on the same problem is the menu being honest. What has to vary is the anchor and the numbers. `_dryrun_drafts.ts` now labels the spread as diagnostic and says what to read it for: the picker refusing outright, or a problem nothing ever selects.

Left open: the concurrency floor is real and its value is unknown, and it must not become a fourth `out_of_scope` condition (a binary test that cannot fire on a book that does not hold peak-concurrency facts is the same mistake as the web-share condition deleted this morning). Also unchased: OVI Technologies scored 0.95 while the picker's rationale cited "live streams with more than 5M viewers", so the live-only veto may have missed it.

**Two "worth a look" notes turned into fixes, same session.** Jake pushed back on my habit of recording a problem instead of solving it, correctly.

**The concurrency floor is now a research question.** `research.always_include` carries "peak concurrent viewers or simultaneous streams on a single title", and the planner authored `open_peak_viewers` (open_web, 365-day window — a concurrency record does not go stale in 30 days). It stays out of `out_of_scope`: a binary test cannot fire on a book holding no concurrency facts, which is the same mistake as the web-share condition deleted this morning. Found while doing it: **`isStrategyFresh()` checks only the age of `strategy_generated_at`.** Nothing watches `guidance` or `always_include` despite the docstring claiming a guidance change regenerates, so editing either field is inert until the cached strategy ages out on its own. Forced the regeneration in the script; the real fix is hashing the planner inputs the way `scoreInputsHash` does for scoring.

**OVI Technologies: the veto was not broken, the evidence was contradictory.** It scored 0.95 while its own site says "when we say Live, we mean LIVE". `scoreEntity` returns null for it, so the judgment was current rather than stale — the rubric had `company_description` saying "sub second live streaming environment" AND an imported `product: Film/TV Streaming` tag asserting a catalog. Condition 1 needs live-only *with no on-demand catalog*, and the block says stop only on evidence rather than assumption, so refusing to veto was right on the evidence it had. Rewording the condition would not fix it and costs a full-book re-rubric anyway.

I also tried to size the class with a keyword sweep and it was worthless: 1343 of 1841 descriptions matched "live-focused" because a VOD service mentioning live once matches, and KKTV and Film Movement Plus came back as candidates. That is the argument for storing the answer rather than re-deriving it, made empirically.

Built what the plan already called for in item 2 and signal-extraction item 5: `policy.enrichment.stable_attributes` (`{predicate, question, values[]}`, empty by default) plus `scripts/backfill_stable_attributes.ts` (dry run default, `--apply` / `--limit` / `--entity`). Nothing about video, live or on-demand appears in code — which properties matter and what they are called is customer config, because the distinction means nothing in another vertical. A value outside the configured list is dropped rather than stored and "unknown" stores nothing, because a wrong stable attribute is worse than a missing one: the scorer trusts it for as long as it stands. Sudden runs `delivery_mode` and `business_model`, the second being exactly the predicate item 2 specified.

Verified on OVI: with `delivery_mode=live_only` stored, the veto fires and cites it, `icp_total` 0.95 → 0, written. Sample of 25 for the full-book decision: `delivery_mode` answered for 7 (3 live_only, 3 both, 1 on_demand_only), `business_model` for 20, nothing rejected as an invented value, 23 honest unknowns storing nothing. If 12% live-only holds across ~1800 candidates that is ~200 accounts wrongly in the book, but the sample is unordered so the rate is indicative only. Not run: it costs ~1800 cheap-model calls plus a rescore of every touched account, the same order as this morning's full rescore, and that is a spend decision rather than mine to make.

**Then the plan's top open item, and the diagnosis in it was wrong.** "Half the top accounts have nothing to write on" was recorded as a research-quality problem: the planner finding company descriptions instead of dated events. Measured it before changing anything, and the data says otherwise.

Research works well when it runs. Since the 07-29 freshness fix, 53% of new research signals come from pages 30 days old or newer, against 9% across the whole corpus. The top accounts that have been researched hold pages 1 to 8 days old. The freshest signals yield `news_event`, `recent_event`, `pain_observed`, `streaming_scale` — event-shaped, not profile-shaped. The corpus only looks profile-heavy because 2536 of its 4226 signals are the CSV import, which is 0% dated by nature.

The actual cap is coverage. **1127 of 1961 accounts have no `attributes.domain`, and a domainless account is never a research candidate by design** — no own-site angle, no contact pull, so a search there is spend with no payoff. ShowMax, the highest-scored account in the book at 1.00, and Cineverse at 0.95 are both in that set with `researched=never`. That is the direct cause of the ShowMax gate that opened this section. `domain-backfill-daily` exists for exactly this, is configured at 75/day, and last ran 2026-08-01 despite a `0 11 * * *` cron. Among the 834 accounts that CAN be researched, 49 distinct ones were touched in 6.8 days: one visit per ~116 days, against a 14-day bar for a trigger-led draft.

Also found: zero `research_triggered` on 08-02 and 08-03, 18 on 08-04, and `domain_resolved` silent since 08-01, while `pipeline_status` reads ok. Same shape as the recurring host/keepalive failure rather than a code bug.

Three levers, none of them a prompt change: get the domain backfill firing again (biggest unlock, doubles the researchable book, ~$11 of Exa over 15 days), shift `selection_mix` toward `high_value` (free), raise `searches_per_run` from its default 30 per tick (~$0.60/day per +10). Left for Jake because two of the three are spend.

**Method note worth keeping.** Two of my diagnostic passes were wrong before they were right, and both failed loudly enough to catch: a signals query selected `signal_type` and `source`, columns that do not exist, so PostgREST errored, `data` came back null, and it printed "0 signals in 30 days" as if the pipeline were dead. And a keyword sweep for live-only accounts matched 1343 of 1841 descriptions because a VOD service that mentions live once matches. Check the shape of a zero and the shape of a suspiciously large number before reporting either.

**Stable-attribute backfill: stopped at 389 of 1914 on Jake's call** (43 live_only, 60 on_demand_only, 33 both, 253 unknown for `delivery_mode`; 344 operates / 3 sells_to / 2 neither / 39 unknown for `business_model`). Those 43 live-only accounts will veto to 0 on their next rescore. Also added a concurrency pool (`--concurrency`, default 5) to the script, since 1914 sequential calls is over an hour of pure network wait.

**And the coverage fix turned out to need no code at all.** Jake's read was that we should just resolve domains for the high-value accounts each day rather than clearing the whole 1127-account backlog. That design already exists: `scanDomainBackfillCandidates` has ordered candidates by `icp_fit` since `9e33a4a` on 07-28. Chased why the top-scored account in the book was still domainless anyway, and it was a sequencing accident rather than a bug: the backfill ran 07-29 to 08-01 against the OLD scores, this morning's full-book rescore is what made ShowMax 1.00 and Cineverse 0.95, and the cron has not fired since 08-01. The ordering only takes effect the next time the job runs.
