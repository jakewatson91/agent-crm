...
- **`agent_logic.ts:408` typecheck failure.** Pre-existing `meta` used-before-declaration in the Hunter pre-flight block. Either hoist `meta` to before the Hunter check or inline the parent_event_id at the callsite. Blocks `pnpm --filter web build`.
