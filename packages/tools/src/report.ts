...
export const DEFAULT_PRICING: Pricing = {
  ...
  // separately — exa_per_content_page stays 0. Only overflow past 10 results/search
  // costs extra ($1/1k results), which we never do.
  exa_per_search: 0.007,
  exa_per_content_page: 0,
...
/** `body_for_embedding` is absent until hydrateBodies fills it, and it only fills
 *  the rows the digest actually quotes. Treat missing as "not fetched", not
 *  "empty". */
...
export interface PeriodData {
  ...
  /** Real billed Exa cost for the window, from Exa's Team Management API, when a
...
/** First-time scores get their own slots. They are summarised as a cohort further
 *  down ("all 17 scored identically on…"), which needs their sub-scores loaded. */
const FIRSTS_RENDER_CAP = 20;
/** Signals quoted under one account, and the count of standalone highlights. Both
 *  bound how many signal bodies collectPeriod has to fetch — see hydrateBodies. */
const SIGNALS_PER_MOVE = 4;
const SIGNAL_HIGHLIGHTS = 5;
/** Flat lists that grow with the window. A 30d digest was emitting a line per
 *  resolved domain and per new contact — hundreds of lines nobody reads. */
const LIST_RENDER_CAP = 12;

/** The signals the "highlights" block quotes. Shared by collectPeriod (to decide
 *  which bodies to fetch) and renderMarkdown (to print them) so the two can't
 *  disagree about which rows need a body. */
function topByMagnitude(signals: SignalRow[], n: number): SignalRow[] {
  return [...signals].sort((a, b) => (b.magnitude ?? 0) - (a.magnitude ?? 0)).slice(0, n);
}
...
export async function collectPeriod(
  ...
  window: PeriodWindow,
  opts: {
    /** Sweep results the caller already has. `pnpm recap` runs the sweep for its
     *  own report; without this the digest would run every check a second time. */
    checks?: CheckResult[];
  } = {},
): Promise<PeriodData> {
  ...
  // No body_for_embedding here: the digest quotes at most ~85 signals but a 30d
  // window holds thousands, at ~2KB of body each. Bodies are fetched by id once
  // we know which ones render (hydrateBodies, below). Same reason migration 0043
  // moved the sweep onto body_hash.
  const signals = await fetchAll<SignalRow>((f, t) =>
    sb.from('signals').select('id, entity_id, magnitude, structured_tags, created_at').eq('workspace_id', wsId).gte('created_at', since).lt('created_at', until).order('created_at', { ascending: true }).range(f, t));
  ...
  // One query for the posts AND the channel→account map they need. The previous
  // version fetched every channel in the workspace (>1000 on a live one) and then
  // asked for posts 150 channel-ids at a time, with no .order() — so a chunk
  // holding more than 1000 posts silently returned only the first 1000. Filtering
  // through the channels FK scopes by workspace directly and pages properly.
  const postRows = await fetchAll<PostRow & { channels: { account_entity_id: string } | Array<{ account_entity_id: string }> | null }>((f, t) =>
    sb.from('channel_posts')
      .select('kind, created_at, body, channel_id, channels!inner(workspace_id, account_entity_id)')
      .eq('channels.workspace_id', wsId)
  ...
  const posts: PostRow[] = postRows.map(({ channels, ...p }) => {
    const ch = Array.isArray(channels) ? channels[0] : channels;
    if (ch?.account_entity_id) accountOfChannel[p.channel_id] = ch.account_entity_id;
    return p as PostRow;
  });
  ...
  // Real billed Exa cost, when a Team Management service key is configured
  // (Settings -> Connectors -> Exa). Falls back to nothing (renderMarkdown then
  // uses the modeled estimate) rather than blocking the digest on Exa being up.
  let exaActual: { totalCostUsd: number | null; error?: string } | null = null;
  const exaServiceKey = resolveEnvVar(policy, 'EXA_SERVICE_API_KEY');
  if (exaServiceKey) {
    const r = await fetchExaActualCost(exaServiceKey, since, until);
    exaActual = r.ok ? { totalCostUsd: r.totalCostUsd } : { totalCostUsd: null, error: r.error };
  }
  // policy.pipeline is a PipelineStatus (policy.ts): the field is `state`, not
  // `status`. Reading the wrong key meant every digest said "running", including
  // the ones written while the whole advance pass sat paused on a credit wall —
  // the one fact the operator most needed off the top of the mail.
  const pipe = policy.pipeline as { state?: string; scope?: string; reason?: string; provider?: string } | undefined;
  const pipelineStatus = pipe?.state === 'paused'
    ? `PAUSED (${pipe.scope ?? 'all'}${pipe.provider ? `, ${pipe.provider}` : ', manual'})${pipe.reason ? ` — ${pipe.reason}` : ''}`
    : (pipe?.state ?? 'running');
  ...
  // One RPC, not one query per scored account. A day that rescores ~2,000 accounts
  // was issuing ~2,000 sequential round-trips here — about 80% of the runtime of
  // the whole digest. latest_facts_before does the DISTINCT ON in Postgres.
  const prevScoreOf: Record<string, string> = {};
  const scoredEntities = [...byEntity.keys()];
  if (scoredEntities.length) {
    const prevRows = await fetchAll<{ subject_entity: string; object_text: string | null }>((f, t) =>
      sb.rpc('latest_facts_before', {
        p_workspace_id: wsId, p_predicate: 'icp_fit', p_cutoff: since,
        p_entities: scoredEntities, p_inclusive: false,
      }).range(f, t));
    for (const r of prevRows) if (r.object_text != null) prevScoreOf[r.subject_entity] = r.object_text;
  }
  ...
  for (const [ent, rows] of byEntity) {
    ...
    const sigs = (signalsByEntity.get(ent) ?? []).map((s) => {
      ...
      return { angle: tags.research_angle ?? tags.signal_source ?? 'signal', magnitude: s.magnitude, body: '', url: tags.url ?? null, at: s.created_at };
    });
  ...
  // An account's first score is not a move — there is nothing it moved from.
  // Ranking everything together on `scoreNew - 0` put every new 0.90 above a real
  // 0.95 → 0.00 drop, so a day of collapsing scores read as a day of good new
  // accounts. The two cohorts answer different questions and get their own room:
  // rescored accounts by how far they went, first scores by how high they landed.
  const movers = candidates.filter((c) => c.scorePrev !== null).sort((a, b) => Math.abs(b.scoreDelta) - Math.abs(a.scoreDelta));
  const firstScores = candidates.filter((c) => c.scorePrev === null).sort((a, b) => b.scoreNew - a.scoreNew);
  const moves = [...movers.slice(0, MOVES_RENDER_CAP), ...firstScores.slice(0, FIRSTS_RENDER_CAP)];
  ...
  // Two batched RPCs replace two queries per account: current breakdown for all of
  // them, plus the pre-window baseline for the ones that had a score already.
  const curBd = await latestBreakdowns(sb, wsId, moves.map((m) => m.entity_id), until, false);
  const prevBd = await latestBreakdowns(sb, wsId, moves.filter((m) => m.scorePrev !== null).map((m) => m.entity_id), since, true);
  for (const m of moves) {
    const cur = curBd.get(m.entity_id) ?? null;
    ...
    if (m.scorePrev !== null) {
      const prev = prevBd.get(m.entity_id) ?? null;
      if (cur && prev) {
  ...
  // Now that the rendered slice is fixed, fetch the handful of signal bodies it
  // quotes — the per-move excerpts and the magnitude highlights, nothing else.
  await hydrateBodies(sb, signals, signalsByEntity, moves);
  ...
  // One COUNT in Postgres. This used to list every account id, then page through
  // them 200 at a time pulling each one's full `attributes` jsonb — megabytes of
  // read to produce a single integer.
  let domainBackfill: { remaining: number; perDay: number } | null = null;
  const perDay = policy.research?.domain_backfill_per_day ?? 0;
  if (perDay > 0) {
    const { data: missing } = await sb.rpc('count_accounts_missing_domain', { p_workspace_id: wsId });
    domainBackfill = { remaining: Number(missing ?? 0), perDay };
  }
  ...
  try {
    const checks = opts.checks ?? await sweepWorkspace(sb, wsId);
...
// Latest icp_fit_breakdown per entity at or before a cutoff, for a batch of
// entities. `exclusive` makes it strictly before (the pre-window baseline).
async function latestBreakdowns(sb: SupabaseClient, wsId: string, ents: string[], cutoff: string, exclusive: boolean): Promise<Map<string, Record<string, unknown>>> {
  const out = new Map<string, Record<string, unknown>>();
  if (!ents.length) return out;
  const rows = await fetchAll<{ subject_entity: string; object_text: string | null }>((f, t) =>
    sb.rpc('latest_facts_before', {
      p_workspace_id: wsId, p_predicate: 'icp_fit_breakdown', p_cutoff: cutoff,
      p_entities: ents, p_inclusive: !exclusive,
    }).range(f, t));
  for (const r of rows) {
    if (!r.object_text) continue;
    try { out.set(r.subject_entity, JSON.parse(r.object_text) as Record<string, unknown>); } catch { /* unparseable breakdown: treat as absent */ }
  }
  return out;
}

/**
 * Fill body_for_embedding on exactly the signals that get quoted: the first few
 * under each rendered account, plus the magnitude highlights. Everything else
 * keeps a null body and never leaves the database.
 *
 * Mutates `signals` rows in place (so the highlights block sees them) and writes
 * the per-move excerpts, which are already trimmed copies.
 */
async function hydrateBodies(sb: SupabaseClient, signals: SignalRow[], signalsByEntity: Map<string, SignalRow[]>, moves: EntityMove[]): Promise<void> {
  const wanted = new Map<string, SignalRow[]>(); // signal id -> rows to patch
  const want = (s: SignalRow) => { const a = wanted.get(s.id) ?? []; a.push(s); wanted.set(s.id, a); };
  for (const s of topByMagnitude(signals, SIGNAL_HIGHLIGHTS)) want(s);
  const perMove = new Map<string, SignalRow[]>();
  for (const m of moves) {
    const rows = (signalsByEntity.get(m.entity_id) ?? []).slice(0, SIGNALS_PER_MOVE);
    perMove.set(m.entity_id, rows);
    for (const s of rows) want(s);
  }
  if (!wanted.size) return;

  const bodyOf = new Map<string, string>();
  for (const ids of chunk([...wanted.keys()], 200)) {
    const { data, error } = await sb.from('signals').select('id, body_for_embedding').in('id', ids);
    if (error) throw error;
    for (const r of (data ?? []) as Array<{ id: string; body_for_embedding: string | null }>) {
      if (r.body_for_embedding) bodyOf.set(r.id, r.body_for_embedding);
    }
  }
  for (const [id, rows] of wanted) {
    const body = bodyOf.get(id);
    if (body) for (const r of rows) r.body_for_embedding = body;
  }
  for (const m of moves) {
    const rows = perMove.get(m.entity_id) ?? [];
    rows.forEach((s, i) => {
      const target = m.signals[i];
      if (target) target.body = (s.body_for_embedding ?? '').replace(/\s+/g, ' ').slice(0, 240);
    });
  }
}
...
export function renderMarkdown(d: PeriodData): string {
  ...
  // A workspace where nothing happened used to render every section anyway —
  // forty lines of zeros per idle workspace, which is most of what a multi-
  // workspace digest was. One line says the same thing. Health still prints,
  // because "quiet" and "broken" look identical from the activity counts alone.
  const scanned = d.advance?.scanned ?? 0;
  const idle = !d.signals.length && !d.facts.length && !d.posts.length && !scanned
    && !d.approvals.pending.length && !d.approvals.decided.length;
  if (idle) {
    const reds = d.health?.checks.filter((c) => c.severity === 'red') ?? [];
    const yellows = d.health?.checks.filter((c) => c.severity === 'yellow') ?? [];
    L.push('', `No activity: no signals, facts, posts, or accounts scanned, and nothing waiting for approval. Health: RED=${reds.length} YELLOW=${yellows.length}.`);
    for (const c of [...reds, ...yellows]) L.push(`- ${c.severity.toUpperCase()} ${c.id}: ${c.metric}`);
    return L.join('\n');
  }
  ...
  // Detail blocks are for accounts that actually changed. The first-time scores
  // are summarised as a cohort below — printing both in full said the same thing
  // twice and buried the changes under it.
  const firsts = d.moves.filter((m) => m.scorePrev === null);
  const changed = d.moves.filter((m) => m.scorePrev !== null);
  H(`What moved and why: ${changed.length} account${changed.length === 1 ? '' : 's'} rescored, ${firsts.length} scored for the first time (of ${fmt(searches)} searches, ${fmt(d.signals.length)} signals)`);
  if (!changed.length) L.push('No account with a prior score changed in this window.');
  for (const m of changed) {
    ...
    if (m.signals.length) {
      ...
      for (const s of m.signals.slice(0, SIGNALS_PER_MOVE)) L.push(`  - [${s.angle}, mag ${s.magnitude ?? '?'}] "${s.body}"${s.url ? ` ${s.url}` : ''}`);
      if (m.signals.length > SIGNALS_PER_MOVE) L.push(`  - …and ${m.signals.length - SIGNALS_PER_MOVE} more signal(s)`);
  ...
  if (firsts.length) {
    ...
    H(`First scores: ${firsts.length} account${firsts.length === 1 ? '' : 's'}, ${lo.toFixed(2)} to ${hi.toFixed(2)}`);
    L.push(firsts.map((m) => `${m.name} ${m.scoreNew.toFixed(2)}`).join(', '));
    // The cohort's own "why", for the few that scored highest — the reasoning is
    // what tells you whether a 0.90 is earned or is the rubric's default shape.
    for (const m of firsts.slice(0, 3)) if (m.reasoning) L.push('', `${m.name} (${m.scoreNew.toFixed(2)}): ${m.reasoning}`);
  ...
  for (const e of resolved.slice(0, LIST_RENDER_CAP)) L.push(`- ${N(e.target_id)} → ${e.payload?.domain}`);
  if (resolved.length > LIST_RENDER_CAP) L.push(`- …and ${resolved.length - LIST_RENDER_CAP} more resolved`);
  ...
  for (const f of newContacts.slice(0, LIST_RENDER_CAP)) L.push(`- ${N(f.subject_entity)} (${roles.get(f.subject_entity) ?? 'role unknown'}, score ${contactScores.get(f.subject_entity) ?? '?'})`);
  if (newContacts.length > LIST_RENDER_CAP) L.push(`- …and ${newContacts.length - LIST_RENDER_CAP} more new contact(s)`);
  ...
  for (const p of drafts.slice(0, LIST_RENDER_CAP)) {
    L.push(`**${N(d.accountOfChannel[p.channel_id])}** (${ts(p.created_at)})`);
    L.push(`> ${(p.body ?? '').replace(/\n/g, ' ')}`, '');
  }
  if (drafts.length > LIST_RENDER_CAP) L.push(`- …and ${drafts.length - LIST_RENDER_CAP} more draft(s)`);
  ...
  // Real billed cost wins when Exa's Team Management API answered (see exaActual
  // in collectPeriod); the modeled per-search estimate is the fallback, not a
  // sanity check on it, so a real $0.00 stays $0.00 even if the model expected more.
  const exaActualUsd = d.exaActual?.totalCostUsd ?? null;
  const exaCostFinal = exaActualUsd ?? exaCost;
  L.push(exaActualUsd != null
    ? `- Exa: ~${exaSearches} searches → ${usd(exaActualUsd)} (actual billed, via Exa API)`
    : `- Exa: ~${exaSearches} searches (~${exaPages} content pages, bundled free under 10/search) → ${usd(exaCost)} (estimated${d.exaActual?.error ? ` — real cost lookup failed: ${d.exaActual.error}` : '; add a Team Management service key in Settings -> Connectors -> Exa for actual billed cost'})`);
  ...
  const total = llmTotal + exaCostFinal + hunterCost;
