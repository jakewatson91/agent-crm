...
/**
 * Entities index — SSR + SWR-with-fallback hybrid, same pattern as the Feed
 * page (see feed/FeedClient.tsx). The server inlines the first payload via
 * `initialData` so the client hydrates with no round-trip and no blank-state
 * flash; SWR then revalidates against the same cached pipeline
 * (app/_lib/entities_index.ts) the server used. Rendering itself still
 * happens entirely client-side — only the data crosses the server boundary —
 * which is what keeps this fast (see the RSC-serialization note in
 * app/_lib/entities_index.ts's sibling API route history: rendering hundreds
 * of entity cards as Server Components was the slow path, not fetching data
 * server-side).
 */
...
import type { EntityRow, Activity, EntitiesPageData } from '../../../_lib/entities_index';
...
export function EntitiesClient({ ws, initialData }: { ws: string; initialData: EntitiesPageData }) {
  const { data, error } = useSWR<EntitiesPageData>(
    `/api/entities/index?workspace_id=${ws}`,
    { ...DEFAULT_SWR, fallbackData: initialData },
  );
