...
/**
 * The rail + hop rendering, split out so the drawer's fact view can show the
 * same provenance trail inline (always expanded) instead of behind CiteChain's
 * own "trace" toggle — one rendering of a chain, two entry points into it.
 */
export function ChainHops({ hops }: { hops: ChainHop[] }) {
  return (
    ...
    <div style={{ display: 'inline-block', width: open ? '100%' : 'auto' }}>
      ...
      {open && (
        <div style={{ marginTop: '.5rem', maxWidth: 640 }}>
          ...
          {chain && <ChainHops hops={chain.hops} />}
        </div>
      )}
    </div>
  );
