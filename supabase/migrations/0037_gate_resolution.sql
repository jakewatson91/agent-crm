-- 0037: persist the human's note + edit diff on a gate decision.
--
-- decide_gate previously recorded only decision/decided_by/decided_at — the
-- reason a human rejected a draft, or what they changed before approving,
-- was used once for a one-line chat audit note and then discarded. This adds
-- a `resolution` jsonb column so that context survives, and pastOutcomes can
-- surface it to future drafts on similar accounts (not just the bare verdict).
--
-- Body is the 0034 version with ONLY the decide_gate branch changed; every
-- other branch is preserved byte-for-byte.

alter table gates add column resolution jsonb not null default '{}'::jsonb;
