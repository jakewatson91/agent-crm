-- agent-crm v0 fix: record_event must handle the create_workspace bootstrap.
-- The caller passes a placeholder p_workspace_id; the function should:
--   1. Allocate the new workspace_id
--   2. Insert the workspace row
--   3. Insert the event with workspace_id = new workspace's id
--
-- Plus: re-grant execute, since changing the function signature recreates it.
...
begin
  ...
  -- Bootstrap: create_workspace must allocate the workspace BEFORE the event,
  -- and the event's workspace_id is the NEW workspace's id, not the placeholder.
