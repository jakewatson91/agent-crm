-- 0038: stop persisting derived embeddings in the event log.
--
-- record_event is the single write path: it appends p_payload to events AND
-- materializes the typed row from that same payload. For create_signal /
-- create_subscription that means the 1536-dim embedding (a ~29 KB JSON string)
-- was stored TWICE — once as a real vector in signals/subscriptions, once as
-- text inside events.payload. The event copy was ~40% of the entire database
-- and made every events.payload read ~5x fatter.
--
-- The embedding is a derived artifact: replay_to (0004) already rebuilds signals
-- WITHOUT it, and nothing reads payload.embedding anywhere. So we trim it from
-- the stored event row only. Materialization is unchanged — the branches below
-- still read the embedding from the incoming p_payload parameter, so the vector
-- columns are populated exactly as before. body_for_embedding stays in the
-- payload (it's the embedding's input and replay reads it).
--
-- Body is the 0037 version with ONLY the events insert changed; every branch is
