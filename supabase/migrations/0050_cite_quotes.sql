-- 0050: structured citation anchors for touch_draft posts.
--
-- CitedText.tsx highlights a draft's cited facts inline by guessing: it takes
-- each fact's raw object_text and searches for a verbatim >=3-word substring
-- in the draft body. Measured against the last 23 real drafts, that guess
-- lands on only 5/23 (13% of individual citations) — the drafter always
-- paraphrases facts into readable sales copy, so the raw fact text almost
-- never appears verbatim.
--
-- Fix: let the drafter say which exact phrase in its own draft reflects each
-- cited fact, at generation time, instead of reverse-engineering it after the
-- fact. cite_quotes is additive and touch_draft-only; cites (fact id list)
-- is untouched so every existing reader keeps working unchanged.
--
-- Body is the 0041 version with ONLY the post_to_channel branch changed
-- (cite_quotes added); every other branch is preserved byte-for-byte.

alter table channel_posts add column cite_quotes jsonb not null default '[]'::jsonb;
...
begin
  ...
  -- Trim derived embeddings out of the stored event payload. Materialization
  -- below still uses p_payload (the parameter), so vector columns are unaffected.
  insert into events (workspace_id, actor_kind, actor_id, action, target_kind, target_id, payload, prompt_hash, parent_event_id)
  values (
    v_workspace_id, p_actor_kind, p_actor_id, p_action, p_target_kind, v_target_id,
    case p_action
      when 'create_signal'       then p_payload - 'embedding'
      when 'create_subscription' then p_payload - 'semantic_embedding'
      else p_payload
    end,
    p_prompt_hash, p_parent_event_id
  )
  ...
  case p_action
    ...
    when 'post_to_channel' then
      insert into channel_posts (id, channel_id, parent_post_id, thread_root_id,
                                 author_kind, author_id, kind, body, cites, cite_quotes, source_event_id,
                                 score_delta)
      values (
        v_target_id,
        ...
        coalesce(p_payload->'cite_quotes', '[]'::jsonb),
        v_event_id,
        (p_payload->>'score_delta')::numeric
      );
    ...
    when 'decide_gate' then
      update gates
         set decided_by = case
             ...
             decided_at = now(),
             resolution = coalesce(p_payload->'resolution', '{}'::jsonb)
