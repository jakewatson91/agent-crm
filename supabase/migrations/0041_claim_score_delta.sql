-- 0041: persist the per-claim score delta.
--
-- The enricher already computes, in memory, exactly how much a claim's facts
-- moved the account's score (score_after - score_before) — then throws it
-- away. There's no need to approximate this from a time-window bucket; it's
-- the real marginal delta of this exact claim. Add a column to capture it at
-- write time, via the same post_to_channel payload that creates the claim.
--
-- Body is the 0038 version with ONLY the post_to_channel branch changed
-- (score_delta added); every other branch is preserved byte-for-byte.

alter table channel_posts add column score_delta numeric;
