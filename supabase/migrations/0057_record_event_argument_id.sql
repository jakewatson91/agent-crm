-- 0057: actually store which argument a draft made.
--
-- 0056 added channel_posts.argument_id and an index on it, and stopped there.
-- The row is not written by application code: it is written by this function,
-- whose insert column list was last set in 0050 and knew nothing about the new
-- column. So the drafter passed argument_id, PostToChannelSchema accepted it,
-- act() forwarded it in the payload, and Postgres dropped it on the floor.
-- Every post had null there.
--
-- That null disabled the limit on an unconfirmed argument, which works by
-- counting the drafts filed under it: it always counted zero. On 2026-08-22 an
-- unconfirmed argument wrote 5 drafts against a limit of 3.
--
-- Body is the 0050 version with ONLY the post_to_channel branch changed
-- (argument_id added); every other branch is preserved byte-for-byte, and this
-- file was generated by copying 0050 rather than retyping it.
