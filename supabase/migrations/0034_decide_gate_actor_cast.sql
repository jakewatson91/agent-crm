-- 0034: stop decide_gate from crashing on non-uuid actor ids.
--
-- record_event's actor_id is text by design — the system uses sentinels like
-- 'web', 'system', and 'source:ats:<id>' as actors, and request_gate already
-- stores p_actor_id into gates.requested_by_agent as plain text. But the
-- decide_gate branch did `decided_by = p_actor_id::uuid`, which throws
-- `invalid input syntax for type uuid: "web"` the moment a human approves a
-- gate from the web UI (actor_id = 'web'). That blocked every outreach send.
--
-- gates.decided_by references auth.users(id) and is only a denormalized
-- convenience — replay reconstructs the decision from the decide_gate event,
-- not from this column. So when the actor isn't a uuid, set decided_by = null
-- and keep the real audit (who/when) in events.actor_id. When a genuine auth
-- user id is passed, it still lands in decided_by as before.
--
-- Body is the 0033 version with ONLY the decide_gate branch changed; every
-- other branch is preserved byte-for-byte.
