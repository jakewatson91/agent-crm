/**
 * Attio benchmark runner.
 *
 * Runs all 3 workloads (draft / brief / score) against the live Attio API
 * for the 6 seeded benchmark accounts, in two shapes:
 *   - default: no attribute selection, full verbose response
 *   - tight:   `attributes` filter to return only fields the drafter needs
 *
...
import { ATTIO_TOOLS, execAttioTool, type AttioShape } from './readers/attio.js';
...
const ATTIO_SHAPES: AttioShape[] = ['default', 'tight'];
...
async function runAttio(workload: Workload, account: string, run: number, shape: AttioShape): Promise<RunRow> {
  try {
    const tStart = Date.now();
    const messages: ChatMessage[] = [
      { role: 'system', content: workload.systemPromptAttio },
      { role: 'user', content: workload.userPromptAttio(account) },
    ];
    ...
    for (let turn = 1; turn <= MAX_TURNS; turn++) {
      const llm = await callDeepSeek(messages, ATTIO_TOOLS);
      llmCalls++;
      totalInput += llm.input_tokens;
      totalOutput += llm.output_tokens;
      saveReceipt({ platform: 'agent-crm', account, run, stage: `attio_${workload.name}_${shape}_turn${turn}_llm`, model: V1_MODEL }, {
        workload: workload.name, shape, platform: 'attio',
      ...
      for (const tc of llm.tool_calls) {
        ...
        const apiCall = await execAttioTool(tc.name, args, shape);
        dataCalls++;
        saveReceipt({ platform: 'agent-crm', account, run, stage: `attio_${workload.name}_${shape}_turn${turn}_tool_${tc.name}` }, { workload: workload.name, shape, platform: 'attio', ...apiCall });
        messages.push({ role: 'tool', tool_call_id: tc.id, content: JSON.stringify(apiCall.response.body) });
      }
    }

    return {
      workload: workload.name, platform: 'attio', shape, account, run,
...
async function main() {
  console.log('Attio benchmark');
  console.log(`model: ${V1_MODEL}`);
  console.log(`accounts: ${ACCOUNTS.length}, shapes: ${ATTIO_SHAPES.join(', ')}, workloads: ${WORKLOAD_NAMES.join(', ')}\n`);
  ...
  for (const workloadName of WORKLOAD_NAMES) {
    ...
    for (const account of ACCOUNTS) {
      for (let run = 1; run <= RUNS_PER_ACCOUNT; run++) {
        for (const shape of ATTIO_SHAPES) {
          const r = await runAttio(workload, account, run, shape);
          newRows.push(r);
          appendFileSync(RUNS_PATH, JSON.stringify(r) + '\n');
          console.log(`  ${workloadName}/attio/${shape.padEnd(7)} ${account.padEnd(14)} run ${run}: ${r.ok ? `${r.input_tokens}in / ${r.output_tokens}out / ${r.llm_calls} LLM / ${r.total_latency_ms}ms` : `FAILED: ${r.error}`}`);
        }
      }
    }
  }
  ...
  console.log(`\n--- New Attio rows: ${newRows.length} (${newRows.filter((r) => r.ok).length} ok, ${newRows.filter((r) => !r.ok).length} failed) ---`);
}
